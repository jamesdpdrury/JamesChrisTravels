...
{ name: "Royal Caribbean", id: "Royal Caribbean July 26" },
...
function sameLocalDay(date1, date2) {
  return (
...
function transform(rows) {
  ...
  rows.forEach(row => {
    ...
    // Share mode hiding
// If column L contains "HIDE"
// then hide this item ONLY in shared mode
...
if (isShareMode() && hideInShare === "HIDE") {
  return;
}

    const startTimestamp = toUTC(
      row["START DATE"],
      row["START TIME"],
      row["START UTC"]
    );

    const endTimestamp = toUTC(
      row["END DATE"],
      row["END TIME"],
      row["END UTC"]
    );
    ...
    if (row.TYPE === "Hotel") {
      phases = { Start: "Check-in", End: "Check-out" };
    }

    if (row.TYPE === "Flight") {
      phases = { Start: "Take off", End: "Land" };
    }

    if (row.TYPE === "Cruise") {
      phases = { Start: "Embarkation", End: "Disembarkation" };
    }

    if (row.TYPE === "Train") {
      phases = { Start: "Depart", End: "Arrive" };
    }
    ...
    if (multiPhaseTypes.includes(row.TYPE)) {
      ...
      // Only show arrival/landing if different day
      if (
        ["Flight", "Train"].includes(row.TYPE) &&
        sameLocalDay(startDateObj, endDateObj)
      ) {
        return;
      }
...
function makeItem(
  ...
  if (type === "Uber") {
    ...
    if (parts.length > 1) {
      title = "Uber to " + parts[1].trim();
    } else {
      title = "Uber to " +
        title.replace(/\bUber\b/i, "").trim();
    }
  }
  ...
  if (type === "Walk") {
    ...
    if (parts.length > 1) {
      title = "Walk to " + parts[1].trim();
    } else {
      title = "Walk to " +
        title.replace(/\bWalk\b/i, "").trim();
    }
  }
  ...
  const includeDetails =
    !(
      multiPhase.includes(type) &&
      phase &&
      !["Check-in","Take off","Embarkation","Depart"].includes(phase)
    );
  ...
  if (row.DETAILS && includeDetails) {
    ...
    switch(type) {
      ...
      case "Hotel":
        detailsFormatted = detailsFormatted.map(
          line => `Room Type: ${line}`
        );
        break;

      case "Flight":
        if (detailsFormatted[0]) {
          detailsFormatted[0] =
            `Flight #: ${detailsFormatted[0]}`;
        }

        if (detailsFormatted[1]) {
          detailsFormatted[1] =
            `Aircraft: ${detailsFormatted[1]}`;
        }

        if (detailsFormatted[2]) {
          detailsFormatted[2] =
            `Cabin: ${detailsFormatted[2]}`;
        }
      ...
      case "Cruise":
        if (detailsFormatted[0]) {
          detailsFormatted[0] =
            `Cruise Line: ${detailsFormatted[0]}`;
        }

        if (detailsFormatted[1]) {
          detailsFormatted[1] =
            `Ship: ${detailsFormatted[1]}`;
        }

        if (detailsFormatted[2]) {
          detailsFormatted[2] =
            `Cabin Type: ${detailsFormatted[2]}`;
        }

        if (detailsFormatted[3]) {
          detailsFormatted[3] =
            `Cabin #: ${detailsFormatted[3]}`;
        }

        if (phase === "Embarkation" && duration) {
      ...
      case "Train":
        if (detailsFormatted[0]) {
          detailsFormatted[0] =
            `Train Company: ${detailsFormatted[0]}`;
        }

        if (detailsFormatted[1]) {
          detailsFormatted[1] =
            `Train #: ${detailsFormatted[1]}`;
        }

        if (detailsFormatted[2]) {
          detailsFormatted[2] =
            `Coach Type: ${detailsFormatted[2]}`;
        }

        if (detailsFormatted[3]) {
          detailsFormatted[3] =
            `Seat #: ${detailsFormatted[3]}`;
        }
...
function render(items, tripName, tripId) {
  ...
  Object.entries(days).forEach(([day, dayItems]) => {
    ...
    section.innerHTML =
      `<h3>${formatPrettyDate(dayDateObj)}</h3>`;
    ...
    dayItems.forEach(item => {
      ...
      // Add duration after start time
      if (
        ["Flight","Train","Bus","Walk"].includes(item.type) &&
        item.duration &&
        ["Take off","Depart",null].includes(item.phase)
      ) {

        durationHTML =
          ` <span class="inline-duration">• ${item.duration}</span>`;
      }
      ...
      el.innerHTML = `
        ...
        <div class="details${placeholderClass}">
          ...
          ${item.bookingRef
            ? `<div class="bookingRef">${item.bookingRef}</div>`
            : ""
          }
...
async function initTripSelector() {
  ...
  const container =
    document.getElementById("trip-buttons-inner");
  ...
  if (isShareMode()) {
    ...
    if (!tripFromUrl) {
      ...
      document.getElementById("timeline").innerHTML =
        `<p style="color:red">No trip specified.</p>`;
  ...
  for (const trip of TRIPS) {
    ...
    try {
      ...
      const realItems =
        items.filter(i => i.type !== "None");

      if (realItems.length === 0) continue;

      const latest = realItems.reduce(
        (max, i) =>
          i.timestamp > max
            ? i.timestamp
            : max,
  ...
  visibleTrips.forEach((trip, index) => {
    ...
    btn.addEventListener("click", async () => {
      ...
      const isOpen =
        cal && cal.style.display !== "none";

      if (isOpen) {

        const rows =
          await fetchSheetRows(trip.id);

        const items = transform(rows);

        const tripStart =
          getTripStartDate(items);
  ...
  if (
    tripFromUrl &&
    visibleTrips.some(t => t.id === tripFromUrl)
  ) {
    ...
    document
      ...
      .forEach(b => {
        ...
        if (b.dataset.tripId === tripFromUrl) {
  ...
  try {
    ...
    if (
      ...
      history.pushState(
        {},
        "",
        `?trip=${encodeURIComponent(tabName)}`
      );
    }
  ...
  for (const trip of TRIPS) {
    ...
    try {
      ...
      realItems.forEach(item => {
        ...
        if (!calendarState.dataByDate[iso]) {
          calendarState.dataByDate[iso] = [];
        }
    ...
    if (tripsOnDay.length > 0) {
      ...
      cell.addEventListener(
        ...
        async () => {
          ...
          const uniqueTripIds = [
            ...new Set(
              tripsOnDay.map(t => t.tripId)
            )
          ];

          if (uniqueTripIds.length === 1) {

            await goToCalendarTripDay(
              uniqueTripIds[0],
              iso
            );
...
async function goToCalendarTripDay(
  tripId,
  iso
) {
...
function showTripChooser(
  ...
  box.innerHTML = `
    ...
    <button class="trip-chooser-cancel">
      Cancel
    </button>
  `;

  const btnWrap =
    box.querySelector(".trip-chooser-buttons");
  ...
  box
    .querySelector(".trip-chooser-cancel")
    .addEventListener("click", () => {
      document.body.removeChild(overlay);
    });
